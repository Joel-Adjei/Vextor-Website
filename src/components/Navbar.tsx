...
export function Navbar() {
  ...
  const bgClass =
    scrolled || !isHome
      ? "bg-[#1a2517]/95 backdrop-blur-md shadow-lg shadow-black/20"
      : "bg-transparent"
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${bgClass}`}>
      <div className="mx-auto max-w-7xl px-6 py-4 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-7">
          {navLinks.map((link) => {
            ...
            if (link.isAnchor) {
              return (
                <a key={link.href} href={link.href} className={`${baseClass} ${colorClass}`}>
                  {link.label}
                </a>
              )
            }
      ...
      >
        <nav className="bg-[#1a2517]/98 backdrop-blur-md px-6 pb-6 flex flex-col gap-4 border-t border-[#acc8a2]/20">
          {navLinks.map((link) => {
            if (link.isAnchor) {
              return (
                <a
          ...
          <Button
            asChild
            className="bg-[#acc8a2] text-[#1a2517] hover:bg-white font-semibold text-sm w-full mt-2"
          >
